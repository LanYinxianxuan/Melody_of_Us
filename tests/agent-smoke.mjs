...
// 覆盖验收标准 11 组场景：普通聊天 / 明显低落 / 拒绝交流 / 开心 / 被责怪 /
// 情绪时间衰减 / 亲近关系安慰 / 连续对话演化 / 禁止过度读心 / 随机事件 JSON / 内置事件池。
// 断言总数 50，其中无恒真断言（阶段 0 已核实：原 3 条 `|| true` 实测 20/20 为真，已改为硬性断言）。
...
const aiOutfile = path.join(os.tmpdir(), `agent-ai-bundle-${process.pid}.mjs`);
buildSync({
...
buildSync({
    entryPoints: [path.join(root, "playground/ai.ts")],
    bundle: true,
    format: "esm",
    platform: "node",
    outfile: aiOutfile,
...
const aiB = await import(aiOutfile + "?t=" + Date.now());
const { normalizeRandomEvent } = aiB;
const ecOutfile = path.join(os.tmpdir(), `agent-ec-bundle-${process.pid}.mjs`);
buildSync({
    entryPoints: [path.join(root, "playground/event-card.ts")],
    bundle: true,
    format: "esm",
    platform: "node",
    outfile: ecOutfile,
});
const ecB = await import(ecOutfile + "?t=" + Date.now());
const { pickDemoEvent } = ecB;
...
resetEnv();
{
    ...
    check("普通聊天不产生事件标记", turn.context.recentEvents.filter(e => e.startsWith("（")).length === 0);
...
resetEnv();
{
    ...
    check("信任尚可时策略含 apologize", turn.strategy.choices.some(c => c.id === "apologize"));
...
resetEnv();
{
    ...
    check("选了 comfort 就必须带不心理咨询约束（not_a_counselor / no_psychoanalysis）",
        !turn.strategy.choices.some(c => c.id === "comfort") || turn.strategy.directives.includes("not_a_counselor") || turn.strategy.directives.includes("no_psychoanalysis"));
}
...
resetEnv();
{
    ...
    check("AI 意愿随对话显著演化（|Δ| > 0.05）", Math.abs(t3.aiAfter.willingness_to_talk - t1.aiAfter.willingness_to_talk) > 0.05);
...
// 随机事件 JSON 规范化（点5：推给 DS 的消息严格 JSON；输出结构校验）
console.log("\n===== 情况 9：随机事件 JSON 规范化 =====");
{
    const good = normalizeRandomEvent({
        title: "一阵风",
        scene: "窗外的风忽然大了起来，桌上的作业纸被吹得哗啦作响，有一张飘到了地上。阳光在扬起的灰尘里打着转，空气里有雨前潮湿的味道。小雨正站在门边回头看你，两人之间隔着两步远，气氛安静了几秒。",
        npc: "小雨先弯下腰捡起纸，抬眼看向你，像在等你的反应。",
    });
    console.log("  title:", good.title, "| scene len:", good.scene.length, "| npc:", good.npc.length);
    check("字段完整（title/scene/npc；无选项耦合）", !!good.title && !!good.scene && !!good.npc && !("options" in good));
    check("scene 细致（>60字，含感官/站位）", good.scene.length >= 60 && /(风|纸|光)/.test(good.scene));

    // 模型若仍输出多余字段（如旧版 options）→ 忽略不报错
    const extra = normalizeRandomEvent(JSON.parse('{"title":"t","scene":"s","npc":"n","options":[{"action":"a","expression":"e"}]}'));
    check("多余字段（options）被忽略，仍返回合法结构", extra.title === "t" && extra.scene === "s" && extra.npc === "n" && !("options" in extra));

    const messy = normalizeRandomEvent({ title: "  老长的标题要被截断成十二个字以内 ", scene: "  场景  ", npc: "  站位  " });
    check("脏数据兜底（标题截断+去空白）", messy.title.length <= 12 && messy.scene === "场景" && messy.npc === "站位");
}

// 内置事件池兜底（无 Key / 模型失败时保证事件卡弹出来）
console.log("\n===== 情况 10：内置事件池（保证弹出） =====");
{
    let ok = true;
    const titles = new Set();
    for (let i = 0; i < 30; i++) {
        const ev = pickDemoEvent();
        if (!ev || !ev.title || !ev.scene || !ev.npc) ok = false;
        if (ev) titles.add(ev.title);
    }
    check("30 次抽取均为完整结构（title/scene/npc）", ok);
    check("事件池有多种事件（>2 个不同标题）", titles.size > 2);
}
